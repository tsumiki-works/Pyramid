...
export class IntDivBlock extends BinopBlock {
    constructor(lr: Vec2) {
        super(lr);
        this.set_content("//");
        this.format();
    ...
    override eval(env: Environment) {
        ...
        if (this_children.length !== 2) {
            throw new Error("pyramid: backend error: int div operation must have two arguments");
        }
        ...
        if (arg1.get_type().type_id === PyramidTypeID.I32 && arg2.get_type().type_id === PyramidTypeID.I32) {
            return ArithmeticOperator.div_int(arg1.eval(env), arg2.eval(env))
        }
...
customElements.define('pyramid-int-div-block', IntDivBlock);