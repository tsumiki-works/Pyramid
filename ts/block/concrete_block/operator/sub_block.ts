...
export class SubBlock extends BinopBlock {
    constructor(lr: Vec2) {
        super(lr);
        this.set_content("-");
        this.format();
    ...
    override eval(env: Environment) {
        ...
        if (this_children.length !== 2) {
            throw new Error("pyramid: backend error: sub operation must have two arguments");
        }
        ...
        if (arg1.get_type().type_id === PyramidTypeID.I32 && arg2.get_type().type_id === PyramidTypeID.I32) {
            return ArithmeticOperator.sub_int(arg1.eval(env), arg2.eval(env))
        }
            ...
            return ArithmeticOperator.sub_float(arg1.eval(env), arg2.eval(env))
        }
        throw new Error("pyramid: backend error: invalid operation ocurred")
    }
}
customElements.define('pyramid-sub-block', SubBlock);