...
export class DefineBlock extends ParentBlock {
    ...
    override determine_pos(x: number, y: number, res: FormatResult) {
        ...
        this.get_children()[0].determine_pos(
            this.get_x(),//offset + res.childrens[0].x,
            y + BlockConst.UNIT_HEIGHT + BlockConst.DEFINE_BLOCK_BORDER,
            res.childrens[0]
        );
        // Function Apply Parts
        this.get_children()[1].determine_pos(
            this.get_x(),
            y + res.childrens[0].bottommost + BlockConst.UNIT_HEIGHT + BlockConst.DEFINE_BLOCK_BORDER * 2,
            res.childrens[1]
        );
        // Make Function Field
