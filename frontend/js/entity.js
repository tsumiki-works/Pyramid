...
function entity_logo() {
    const pos = convert_2dscreen_to_2dunnormlizedviewport(canvas.width, canvas.height, [LOGO_WIDTH * 0.5, LOGO_HEIGHT * 0.5]);
    return [
        [pos[0], pos[1], 0.0],
...
function entity_menu() {
    ...
    const pos = convert_2dscreen_to_2dunnormlizedviewport(canvas.width, canvas.height, [MENU_WIDTH * 0.5, MENU_HEIGHT * 0.5]);
    return [
...
function entity_console() {
    const pos = convert_2dscreen_to_2dunnormlizedviewport(canvas.width, canvas.height, [0, CONSOLE_HEIGHT * 0.5]);
    return [
