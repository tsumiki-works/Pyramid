...
pub fn diff(f: fn(f64) -> f64, x: f64) -> Result<f64, String> {
    if x.is_infinite() || x.is_nan() {
        return Err(String::from(
            "pyramid backend error: argument must be finite number.(invailed argument detected)",
        ));
    }
    let h = f64::EPSILON;
    ...
    if ans.is_infinite() {
        return Err(String::from(
            "pyramid backend error: function value must be finite",
        ));
    } else if ans.is_nan() {
        return Err(String::from(
            "pyramid backend error: invalid operation was performed.(NAN detected)",
        ));
    } else {
        Ok(ans)
    }
...
pub fn integrate(f: fn(f64) -> f64, a: f64, b: f64) -> Result<f64, String> {
    ...
    if a > b {
        return Err(String::from(
            "pyramid backend error: invalid integration interval",
        ));
    }
    let split_num = search_split_num(1, f, f64::MAX, f64::INFINITY, a, b)?;
    let ans = ((b - a) / (2. * (split_num as f64)))
        * (2. * lin_space(a..=b, split_num + 1).fold(0., |acc: f64, val| acc + f(val))
            - f(a)
            - f(b));
    if ans.is_infinite() {
        return Err(String::from(
...
fn search_split_num(
    ...
    let solution = ((b - a) / (2. * (split_num as f64)))
        * (2. * lin_space(a..=b, split_num + 1).fold(0., |acc: f64, val| acc + f(val))
            - f(a)
            - f(b));
    ...
    let rel_err = if solution == 0. {
        (solution - last_solution).abs()
    } else {
        ((solution - last_solution) / solution).abs()
    };
